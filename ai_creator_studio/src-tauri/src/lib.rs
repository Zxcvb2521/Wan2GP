...
pub fn start_engine(app: AppHandle, state: State<'_, RuntimeState>) -> Result<String, String> {
    ...
    if !python.exists() {
        return Err(format!("Встроенный Python не найден: {}", python.display()));
    }
    if !entry.exists() {
        return Err(format!("Backend AI Creator не найден: {}", entry.display()));
    }

    let mut guard = state.child.lock().map_err(|_| "Не удалось получить состояние движка")?;
    if guard.is_some() {
        return Ok("Движок уже запущен".into());
    }
...
pub fn stop_engine(state: State<'_, RuntimeState>) -> Result<String, String> {
    ...
    if let Some(mut child) = guard.take() {
        let _ = child.kill();
        let _ = child.wait();
        return Ok("Движок остановлен".into());
    }
