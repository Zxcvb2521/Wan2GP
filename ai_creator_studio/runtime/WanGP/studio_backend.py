...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  try:
   ...
   elif p.path.startswith("/jobs/"):job=_jobs.get(p.path.rsplit("/",1)[-1]);self.send_json(200 if job else 404,{k:v for k,v in job.items() if k!="session_job"} if job else {"error":"Задача не найдена"})
   elif p.path.startswith("/renders/"):job=_render_jobs.get(p.path.rsplit("/",1)[-1]);self.send_json(200 if job else 404,job or {"error":"Рендер не найден"})
 ...
 def do_PUT(self):
  try:
   ...
   if "/timeline/items/" in p.path:
    parts=p.path.strip("/").split("/");pid=parts[1];item_id=parts[3];self.send_json(200,{"ok":True,"timeline":get_timeline().update_item(pid,item_id,body)});return
   self.send_json(404,{"ok":False,"error":"Не найдено"})
 ...
 def do_DELETE(self):
  try:
   p=urlparse(self.path)
   if "/timeline/items/" in p.path:
    parts=p.path.strip("/").split("/");pid=parts[1];item_id=parts[3];self.send_json(200,{"ok":True,"timeline":get_timeline().delete_item(pid,item_id)});return
   self.send_json(404,{"ok":False,"error":"Не найдено"})
  except KeyError as exc:self.send_json(404,{"ok":False,"error":str(exc)})
  except Exception as exc:self.send_json(500,{"ok":False,"error":str(exc)})
