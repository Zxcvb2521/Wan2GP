...
def _nvidia() -> dict[str, Any] | None:
    ...
    try:
        ...
        if rows:
            gpu = rows[0]
            gpu.update(vendor="NVIDIA", backend="CUDA", gpus=rows)
            return gpu
    except Exception:
        pass
    return None
...
def _other_gpu() -> dict[str, Any] | None:
    ...
    try:
        ...
        if names:
            ...
            return {"name": name, "vendor": vendor, "backend": "CPU/OS", "vram_gb": None, "driver": None, "gpus": [{"name": x, "vendor": vendor} for x in names]}
    except Exception:
        pass
    return None
...
def detect_hardware() -> dict[str, Any]:
    ...
    return {
        ...
        "gpu": gpu,
        "profile": profile,
        "accelerator_available": bool(gpu and gpu.get("vendor") == "NVIDIA"),
    }
